...
{
    $base64 = str_split('ABCDEFGHIJKLMNOPQRSTUVWXYZ!@#$%^&*()-_+[{}],<.>/?\|~0123456789+/');
    $bit_pattern = '';
    $padding = 0;
    $encoded = '';
...
function decodecharacter($str)
{
    $bit_pattern = '';
    $nilai = str_split($str);
    $nilaiasli =  str_split("abcdefghijklmnopqrstuvwxyz");
    $validasi = str_split('!@#$%^&*()-_+[{}],<.>/?\|~');
    $base64 = str_split('ABCDEFGHIJKLMNOPQRSTUVWXYZ!@#$%^&*()-_+[{}],<.>/?\|~0123456789+/');
    foreach (str_split($str) as $b64_encoded) {
        if($validasi[$b64_encoded] = $str)
        print_r($bit_pattern);
    }
    
}
decodecharacter('anjaykoko');